...
# ============================================================
# HssN PanelL Universal Installer / Manager
# ============================================================

APP_DIR="${SPIDER_APP_DIR:-/opt/SpiderPanel}"
REPO="${SPIDER_REPO:-https://github.com/code002h/hassan-panel.git}"
BRANCH="${SPIDER_BRANCH:-main}"

INSTALLER_URL="${SPIDER_INSTALLER_URL:-https://raw.githubusercontent.com/code002h/hassan-panel/main/start.sh}"
...
start_panel() {
    ...
    if systemd_ok; then
        ...
        if running; then
            ...
            ok "HssN PanelL already running."
    ...
    if running; then
        ...
        ok "HssN PanelL is running."

        return 0
    fi


    printf '[ERROR] HssN PanelL failed to start.\n' >&2
...
uninstall_panel() {
    ...
    echo "This will remove HssN PanelL."
...
if [[ ! -f "$APP/start.sh" ]]; then
    echo "HssN PanelL is not installed."
    exit 1
fi
...
main() {
    ...
    case "${1:-install}" in
        ...
        *)
            ...
            echo
            echo "HssN PanelL Universal Installer"
            echo
            echo "Usage:"
            echo
